...
open struct
  module Inner = struct
    ...
    (* Bottom element: no ownership information available *)
    let bottom = None

    (* Top element: empty map allowing all possible ownerships *)
    ...
    let leq d1 d2 =
      if Option.is_none d1 then
        true (* bottom ≤ everything *)
      else if Option.is_none d2 then
        false (* nothing ≤ bottom except bottom *)
      else (
        ...
        Sym.Map.for_all
          (fun x (front, back) ->
             ...
             | Some (front', back') -> front >= front' && back >= back' (* wider bounds *)
             | None -> true) (* symbol not constrained in d2 *)
          d1)
        ...
        Some
          (Sym.Map.merge
             (fun _ o1 o2 ->
                ...
                | None, _ | _, None -> None) (* symbol must be present in both *)
             d1
             d2)
      | None, d | d, None -> d (* join with bottom gives the other element *)
    ...
    let pp d =
      let open Pp in
      match d with
      | Some d when Sym.Map.is_empty d -> !^"⊤" (* top element *)
      | Some d ->
        (* Format each symbol's ownership as "symbol owns X bytes in front and Y bytes after" *)
        separate_map
          (semi ^^ break 1)
          (fun (x, (front, back)) ->
             Sym.pp x
    ...
    | `Call (fsym, _) | `CallSized (fsym, _, _) ->
      (match Sym.Map.find_opt fsym ctx with
       ...
       | None ->
         ...
         d)
    (* Assignment statements: not yet implemented *)

...
module WrappedIntervalBasis = struct
  ...
  let lex_leq x y =
    let a = Z.gt x Z.zero in
    let b = Z.gt y Z.zero in
    if (not a) && b then
      false
  ...
  let wrapped_member e start stop bt =
    ...
    let modulus = Z.shift_left Z.one width in
    let normalize z =
      let result = Z.rem z modulus in
      if Z.lt result Z.zero then Z.add result modulus else result
    in
  ...
  let is_top { bt; is_bottom; start; stop; _ } =
    if is_bottom then
      ...
      (Z.equal start min && Z.equal stop max) || Z.equal (Z.sub start stop) Z.one)
  ...
  let leq s1 s2 =
    ...
    if s1.is_bottom then
      ...
      false
    else (* Check if both endpoints of s1 are in s2 and s1 has ≤ elements than s2 *)
      wrapped_member s1.start s2.start s2.stop s1.bt
      && wrapped_member s1.stop s2.start s2.stop s1.bt
      && Z.leq (cardinality s1.bt s1.start s1.stop) (cardinality s2.bt s2.start s2.stop)
  ...
  let join s1 s2 =
    ...
    if s1.is_bottom then
      ...
      (* Non-overlapping cases with tie-breaking *)
        ...
        let card_bc = cardinality s1.bt b c in
        let card_da = cardinality s1.bt d a in
  ...
  (* Helper: normalize a Z.t value to the bit-width of the domain *)
  let normalize bt z =
    let width = get_width bt in
    let modulus = Z.shift_left Z.one width in
    Z.rem (Z.add z modulus) modulus
  ...
  let make_interval bt start stop =
    ...
    if Z.equal (cardinality bt start stop) Z.zero && not (is_top s) then
      bottom bt
    else
      s
  ...
  let meet s1 s2 =
    ...
    if s1.is_bottom || s2.is_bottom then
      ...
      (* If one covers the other, the meet is not convex. We return the one with smallest cardinality *)
        ...
        && wrapped_member d a b s1.bt
      then (
        let card1 = cardinality s1.bt a b in
        let card2 = cardinality s2.bt c d in
  ...
  let bigger r1 r2 =
    if r1.is_bottom then
      ...
      let card1 = cardinality r1.bt r1.start r1.stop in
      let card2 = cardinality r2.bt r2.start r2.stop in
  ...
  let maximal_under_approximation_of_union (intervals : t list) : t =
    ...
    | _ :: _ ->
      ...
      if List.is_empty non_bottom_intervals then
        ...
        let sorted_intervals =
          List.sort (fun r1 r2 -> Z.compare r1.start r2.start) non_bottom_intervals
        in
  ...
  let meet_many intervals =
    ...
    | _ :: _ ->
      ...
      let union_of_complements_under_approx =
        maximal_under_approximation_of_union complements
      in
  ...
  let wrapped_remainder bt dividend divisor is_signed =
    ...
    if Z.equal dividend.start Z.zero && Z.equal dividend.stop Z.zero then
      ...
      (* Signed case: south pole cut and compute for each element of Cartesian product *)
      ...
      if List.is_empty s2 then
        bottom bt (* Sanity check: empty means interval [0,0] *)
      else (
          ...
          (fun d1 ->
             List.iter
               (fun d2 ->
                  ...
                  let c = d2.start in
                  let d = d2.stop in
                  let is_zero_a = is_msb_zero bt a in
                  let is_zero_c = is_msb_zero bt c in
                  let lb, ub =
                    if is_zero_a && is_zero_c then (* Both positive: [0, d-1] *)
                      (Z.zero, Z.sub d Z.one)
                    else if is_zero_a && not is_zero_c then
                      (* Dividend positive, divisor negative: [0, -c-1] *)
                      (Z.zero, Z.sub (Z.neg c) Z.one)
                    else if (not is_zero_a) && is_zero_c then
                      (* Dividend negative, divisor positive: [-d+1, 0] *)
                      (Z.add (Z.neg d) Z.one, Z.zero)
                    else (* not is_zero_a && not is_zero_c *)
                      (* Both negative: [c+1, 0] *)
                      (Z.add c Z.one, Z.zero)
                  in
    ...
    if s1.is_bottom || s2.is_bottom then
      ...
      if is_zero s1 || is_zero s2 then
        ...
        List.iter
          ...
          s1_splits;
        (* Join all results *)
        List.fold_left join (bottom bt) !results))
  ...
  let forward_abs_binop (op : IT.binop) (b1 : t) (b2 : t) : t option =
    ...
    if b1.is_bottom || b2.is_bottom then
      ...
      | Add ->
        ...
        let card1 = cardinality b1.bt b1.start b1.stop in
        let card2 = cardinality b2.bt b2.start b2.stop in
    ...
    | Cast (dst_bt, _) ->
      ...
      if b.is_bottom then
        ...
        if src_width = dst_width then (* Same width - just change type *)
          ...
          let card = cardinality b.bt b.start b.stop in

...
module Make (GT : GenTerms.T) (I : Domain.T with type t = GT.AD.t) = struct
  ...
  let annotate_gd (abs_ctx : AD.t Sym.Map.t) (defs_ctx : Ctx.t) (gd : Def.t)
    ...
    let rec aux
      ...
      | `EagerDomain _ -> failwith ("unreachable @ " ^ __LOC__)
      | `Return _ ->
      ...
      | `Force ((x, gt_inner), gt') ->
        ...
        let gt', d_list = aux abs_ctx defs_ctx gt' d should_assert in
        (GT.force_ ((x, gt_inner), gt') tag loc, d_list)
      | `ForceElab (backtrack_var, (x, gt_inner), gt') ->
        (* Invisible to abstract interpreter *)
        let gt', d_list = aux abs_ctx defs_ctx gt' d should_assert in
        (GT.force_elab_ (backtrack_var, (x, gt_inner), gt') tag loc, d_list)
      | `LetStar ((x, gt1), gt2) ->

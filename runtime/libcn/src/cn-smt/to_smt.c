...
static sexp_t* maybe_name(sexp_t* e, sexp_t* (*k)(sexp_t*));
static sexp_t* default_term(struct cn_smt_solver* s, cn_base_type bt);
...
/** Helper function implementing OCaml's maybe_name logic */
static sexp_t* maybe_name(sexp_t* e, sexp_t* (*k)(sexp_t*)) {
  if (sexp_is_atom(e)) {
    return k(e);
  } else {
    ...
    sexp_t* result_expr = k(x_atom);
    sexp_t* bindings[] = {sexp_list((sexp_t*[]){x_atom, e}, 2)};
...
static sexp_t* default_term(struct cn_smt_solver* s, cn_base_type bt) {
  cn_term* default_const = cn_smt_default(bt);
  sexp_t* result = translate_term(s, default_const);
  // Clean up default_const if needed
  return result;
}

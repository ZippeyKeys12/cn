...
let test_interval_cast_overflow _ =
  ...
  assert_equal ~msg:"Cast overflow: bt should be target type" bt8 (IntervalBasis.bt result)
...
let test_interval_cast_same_type _ =
  ...
  assert_bool "Cast to same type should not be bottom" (not (IntervalBasis.is_bottom result));
...
let test_interval_cast_unsigned_to_signed _ =
  ...
  assert_equal ~msg:"Cast u32->s32: bt should be target type" bt_s32 (IntervalBasis.bt result)
...
let test_interval_cast_narrow_to_wide _ =
  ...
  assert_bool "Cast narrow->wide should not be bottom" (not (IntervalBasis.is_bottom result));
  ...
  assert_equal ~msg:"Cast narrow->wide: bt should be target type" bt32 (IntervalBasis.bt result)
...
let cast_soundness_prop =
  Test.make
    ...
    (fun (start_val, stop_val) ->
       ...
       let interval = IntervalBasis.of_interval source_bt (Z.of_int min_val) (Z.of_int max_val) in
       ...
       let basic_props =
         ...
         (IntervalBasis.is_bottom result || (Z.leq target_min result.start && Z.leq result.stop target_max))
       in
       
       (* If no overflow occurs, the values should be preserved *)
       let overflow_check =
         if Z.geq (Z.of_int min_val) target_min && Z.leq (Z.of_int max_val) target_max then
           (* No overflow expected *)
           (not (IntervalBasis.is_bottom result)) &&
           Z.equal result.start (Z.of_int min_val) &&
           Z.equal result.stop (Z.of_int max_val)
         else
           (* Overflow expected - should be top *)
...
let cast_identity_prop =
  Test.make
    ...
    (fun (start_val, stop_val) ->
       let bt = BT.Bits (Signed, 32) in
       let min_val, max_val = (min start_val stop_val, max start_val stop_val) in
       let interval = IntervalBasis.of_interval bt (Z.of_int min_val) (Z.of_int max_val) in
       let result = IntervalBasis.cast interval bt in
...
let suite =
  ...
  >::: [ unit_tests; property_tests; arithmetic_tests; arithmetic_property_tests; cast_tests; cast_property_tests ]
